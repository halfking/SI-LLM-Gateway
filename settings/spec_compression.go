	...
	return []*Spec{
		{
			...
			Options:         []string{"off", "auto_threshold", "on_4xx", "delta_only", "smart", "aggressive", "headroom", "headroom_aggressive"},
			Default:         "smart",
			Description:     "压缩模式",
			DescriptionLong: "off=从不压缩；auto_threshold=请求前按阈值压缩；on_4xx=仅在 upstream 返回 context_length_exceeded 后压缩；delta_only=仅保留增量（无会话压缩）；smart=v4 智能压缩（默认，含工具裁剪+滑动窗口）；aggressive=v4 激进压缩（含任务分析+主动压缩）；headroom=v7 Headroom 压缩（JSON 数组智能压缩）；headroom_aggressive=v7 Headroom 激进模式（更激进的数组压缩+CCR 存储）。",
			Unit:            "",
			DangerLevel:     Warning,
			HotReload:       true,
