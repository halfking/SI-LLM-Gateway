// Auto-translated draft (ar-SA) · 2026-07-02 · please review
// nav.ts — نصوص التنقل في الشريط الجانبي، المطابقة لـ NAV_PRIMARY_ITEMS / NAV_GROUPS في config/appNav.ts.
// تسمية المفاتيح: nav.<group>.<item>، مطابقة لـ labelKey في appNav.
export default {
  collapseSidebar: 'طي الشريط الجانبي',
  expandSidebar: 'توسيع الشريط الجانبي',

  group: {
    tenantPortal: 'خدماتي',
    modelsRouting: 'النماذج والتوجيه',
    tenantUsers: 'مستخدمو المستأجر',
    requestsSessions: 'الطلبات والجلسات',
    dataOps: 'عمليات البيانات',
    opsplatform: 'منصة العمليات',
    guide: 'دليل التكامل',
    chat: 'محادثة',
  },

  item: {
    overview: 'نظرة عامة',
    tenantModels: 'النماذج القياسية',
    tenantAccount: 'حسابي',
    tenantPricing: 'الباقات والشحن',
    tenantUsage: 'استهلاكي',
    models: 'النماذج والكتالوج',
    routingOverview: 'نظرة التوجيه',
    credentialMonitor: 'مراقبة بيانات الاعتماد',
    probeHealth: 'صحة الاستكشاف',
    providers: 'المزودون',
    pricing: 'تكلفة التسعير',
    modelPricing: 'إدارة التسعير',
    freePool: 'الموارد المجانية',
    tenants: 'إدارة المستأجرين',
    users: 'إدارة المستخدمين',
    keys: 'مفاتيح API',
    keyApplications: 'طلب المفتاح',
    auditLogs: 'سجلات التدقيق',
    requestLogs: 'سجلات الطلبات',
    sessions: 'قائمة الجلسات',
    sessionManagement: 'إدارة الجلسات',
    sessionCompare: 'مقارنة الجلسات',
    compression: 'إدارة الضغط',
    wechatBot: 'بوت WeChat',
    sessionContext: 'سياق الجلسة',
    settings: 'إعدادات النظام',
    dataLifecycle: 'دورة حياة البيانات',
    formatAnomalies: 'مراقبة شذوذ التنسيق',
    modules: 'إدارة الوحدات',
    sessionAnalytics: 'مركز تحليل الجلسات',
    sessionClusters: 'تجميع الجلسات',
    sessionAudit: 'تدقيق الجلسات',
    sessionConfig: 'إعدادات الجلسة',
    agents: 'سجل الوكلاء',
    promptInjection: 'Prompt Injection Detection',
    examples: 'أمثلة التكامل',
    chat: 'محادثة',
    opsOverview: 'نظرة عامة على العمليات',
    opsLicenses: 'إدارة التراخيص',
    opsDownloads: 'الإصدارات والتنزيلات',
    opsFaults: 'إدارة الأعطال',
    opsAutoUpdate: 'التحديث التلقائي',
    opsCenter: 'عمليات المركز',
    opsVibeCoding: 'VibeCoding',
    opsProductEntry: 'مدخل المنتج',
  },
}
