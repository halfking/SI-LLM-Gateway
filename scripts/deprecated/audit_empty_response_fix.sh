...
if docker ps | grep -q r112_postgres; then
    ...
    PGPASSWORD="${PGPASSWORD:-<db-password>}" psql -h 127.0.0.1 -p 5432 -U [DB_USER] -d postgres \
    ...
    PGPASSWORD="${PGPASSWORD:-<db-password>}" psql -h 127.0.0.1 -p 5432 -U [DB_USER] -d llm_gateway_audit_test \
        -f scripts/bootstrap_full_schema.sql 2>&1 > /dev/null
    
    # 运行集成测试
    echo "  - 运行集成测试"
    export LLM_GATEWAY_PG_URL="postgres://[DB_USER]:${PGPASSWORD:-<db-password>}@127.0.0.1:5432/llm_gateway_audit_test?sslmode=disable"
